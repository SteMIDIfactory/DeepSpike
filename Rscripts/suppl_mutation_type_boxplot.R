...
avoid1<-unique(grep("Del",merged$variable,value = T))
avoid2<-unique(grep("In",merged$variable,value = T))
...
ggplot(merged,aes(x=variable,y = value,fill=patient))+
  geom_boxplot()+facet_grid(~variable,scale = "free")+
  geom_text(data = merged,aes(label = ifelse(V2<0.05, "*", ""),y = 45),color="red",size=10)+
